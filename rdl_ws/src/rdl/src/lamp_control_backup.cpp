...
class SkypeInterface {
  ...
  void ReceiveMsg(const std_msgs::String::ConstPtr& msg) {
	if(Match(msg->data.c_str(), "CHATMESSAGE.+STATUS\\sRECEIVED")) {
		ROS_INFO("I received a Skype Message!!!");
		init_mov[0] = true;
	}
  }
...
int main(int argc, char **argv)
{
  ...
  if (strcmp(*(argv+1), rec) == 0) {
	...
	std::string rec_file = "/home/fabian/ROS/rdl/data/";
  ...
  if (strcmp(*(argv+1), play) == 0) {
	...
	std::string play_file = "/home/fabian/ROS/rdl/data/";
  ...
  while(ok == true)
  {    
    if(skype.init_mov[0]==true) {
	if (move_up_up(w, &t, &t_start)==false) {
    		t_start = 0;
    ...
    //Kreis
    //w[1] = 0.07 * sin(2*PI*f_sin*t+PI/2);
    //w[2] = 0.219 + 0.07 * sin(2*PI*f_sin*t);

    //Auf Linie entlang x bei y=z=const
    //w[0] = 0.108 + 0.09 * sin(2*PI*f_sin*t);

    //Auf Linie entlang z bei x=y=const
    //w[2] = 0.219 + 0.05 * sin(2*PI*f_sin*t);
    ...
    /*
    current_q[0] = servos.joint_state[0].current_pos;
    current_q[1] = servos.joint_state[1].current_pos;
    current_q[2] = - servos.joint_state[2].current_pos;
    fkin.Jnt2Cart(current_q);
    fkin.Jnt2JacInv(current_q);

    if (abs(0.28-fkin.w[2])<0.005) w[2]=0.16;
    if (abs(0.16-fkin.w[2])<0.005) w[2]=0.28;
    for(int i=0; i<3; i++) ROS_INFO("W[%i]=%lf", i, fkin.w[i]);


    for(int i=0; i<3; i++) cart_dir[i] = w[i] - fkin.w[i];
    abs_cart_dir = cart_dir[0] * cart_dir[0] + cart_dir[1] * cart_dir[1] + cart_dir[2] * cart_dir[2];
    for(int i=0; i<3; i++) cart_dir[i] /= abs_cart_dir;
    
    //for(int i=0; i<3; i++) ROS_INFO("cart_dir[%i]: %lf", i, cart_dir[i]);


    for(int i=0; i<3; i++) {
	velocities[i] = (fkin.J_inv[i][0] * cart_dir[0] + fkin.J_inv[i][1] * cart_dir[1] + fkin.J_inv[i][2] * cart_dir[2]) * K;
	velocities[i] = abs(velocities[i]);
    	ROS_INFO("Joint Velocity %i: %lf", i, velocities[i]);
    }
    ...
    */
    ...
    //q[0] = PI/2 * sin(2*PI*f_sin*t+PI/2);
    //q[1] = PI/2 * sin(2*PI*f_sin*t+PI/2);
